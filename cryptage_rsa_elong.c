...
int main (int argc, char** argv)  {
    ...
    while (read(fdi, &caractere,1)>0) {
  		 codeAscii.l=(long long) caractere;
         code=exponentiation(codeAscii,e,n);
  		 printf("code ascii: %lld . %lld code: %lld . %lld caractere: %c\n",
            codeAscii.h,codeAscii.l, code.h,code.l, caractere);
  		 write(fdo, &code,sizeof(elong));
	}
	close(fdi);
    close( fdo );
